...
if speed != 1.0:
    wav = {p: AF.resample(torch.from_numpy(x), int(round(16000 * speed)), 16000).numpy() for p, x in wav.items()}
...
(pub / "dev" / "runs" / run / f"rescore_{'_'.join(cand_runs)}_s{speed}.json").write_text(json.dumps({"refs": list(val.t), "cands": out}, ensure_ascii=False), encoding="utf-8")
